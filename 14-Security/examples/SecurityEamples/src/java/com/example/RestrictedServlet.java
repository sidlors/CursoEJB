...
// all HTTP methods require "admin" or "member"
@ServletSecurity(@HttpConstraint(rolesAllowed = {"admin", "member"}))
// allow all HTTP methods except POST
// @ServletSecurity(httpMethodConstraints = @HttpMethodConstraint(value="POST", emptyRoleSemantic = EmptyRoleSemantic.DENY))
...
@WebServlet(name = "RestrictedServlet", urlPatterns = {"/RestrictedServlet"})
public class RestrictedServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet RestrictedServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet RestrictedServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        } finally {            
            out.close();
